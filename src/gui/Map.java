...
public class Map extends JPanel {
	...
	public Map(Game theGame) {
		...
		for (int x = 0; x < mazeHeight; x++) {
			for (int y = 0; y < mazeWidth; y++) {
				...
				if(myGame.getGameState().getMyRooms()[x + 1][y + 1].getMyWest()) {
					rooms[x][y].markLeftOpen();
				} else {
					rooms[x][y].markLeftClosed();
				}
				
				if(myGame.getGameState().getMyRooms()[x + 1][y + 1].getMyEast()) {
				...
				if(myGame.getGameState().getMyRooms()[x + 1][y + 1].getMyNorth()) {
					rooms[x][y].markUpperOpen();
				} else {
					rooms[x][y].markUpperClosed();
				}
				
				if(myGame.getGameState().getMyRooms()[x + 1][y + 1].getMySouth()) {
	...
	public void updateVisuals() {
		...
		// remark all paths
		for(int x = 0; x < myGame.getGameState().getMazeHeight(); x++) {
			for(int y = 0; y < myGame.getGameState().getMazeWidth(); y++) {
				if(myGame.getGameState().getMyRooms()[x + 1][y + 1].getMyWest()) {
					rooms[x][y].markLeftOpen();
				} else {
					rooms[x][y].markLeftClosed();
	...
	public void getPlayerMovement() {
		...
		int x = myGame.getGameState().getMyRow();
		int y = myGame.getGameState().getMyCol();
		
		if(myGame.getGameState().getMyRooms()[x][y].getMyNorth()) {
			rooms[myGame.getGameState().getMyRow() - 2][myGame.getGameState().getMyCol() - 1].enableNavigation(Direction.NORTH);
			movementAvailable = true;
		}
		
		if(myGame.getGameState().getMyRooms()[x][y].getMySouth()) {
			rooms[myGame.getGameState().getMyRow()][myGame.getGameState().getMyCol() - 1].enableNavigation(Direction.SOUTH);
			movementAvailable = true;
		}
	
		if(myGame.getGameState().getMyRooms()[x][y].getMyWest()) {
			rooms[myGame.getGameState().getMyRow() - 1][myGame.getGameState().getMyCol() - 2].enableNavigation(Direction.WEST);
			movementAvailable = true;
		}
	
		if(myGame.getGameState().getMyRooms()[x][y].getMyEast()) {
			rooms[myGame.getGameState().getMyRow() - 1][myGame.getGameState().getMyCol()].enableNavigation(Direction.EAST);
			movementAvailable = true;
		}
